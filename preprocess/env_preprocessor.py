...
class EnvPreprocessor(GenericEuroStatPreprocessor):
    def _flatten(self, df: DataFrame) -> DataFrame:
        ...
        new_df = pd.pivot_table(
            ...
            columns=["ceparema", "env_econ", "unit"],
            values="value",
            aggfunc="first",
        )
        new_df.reset_index(level=["location", "timestamp"], inplace=True)
        new_df.columns = new_df.columns.to_flat_index()
        new_df.columns = [("env:" + column[0] + ":" + column[1] + ":" + column[2]).lower() for column in new_df.columns]
        new_df.rename(columns={"env:location::": "location", "env:timestamp::": "timestamp"}, inplace=True)
